...
puts describe "./lib/game_status.rb" do
  ...
  puts describe "#won?" do
    ...
    puts it 'returns false for a draw' do
      board = ["X", "O", "X", "O", "X", "X", "O", "X", "O"]

      expect(won?(board)).to be_falsey
    end

    puts it 'returns an array of matching indexes for a top row win' do
      board = ["X", "X", "X", "O", "O", " ", " ", " ", " "]

      expect(won?(board)).to match_array([0,1,2])
    end

    puts it 'returns an array of matching indexes for a middle row win' do
      board = ["O", "O", " ", "X", "X", "X", " ", " ", " "]

      expect(won?(board)).to match_array([3,4,5])
    end

  puts  it 'returns an array of matching indexes for a bottom row win' do
      board = [" ", " ", " ", "O", "O", " ", "X", "X", "X"]

      expect(won?(board)).to match_array([6,7,8])
    end

  puts  it 'returns an array of matching indexes for a left column win' do
      board = ["O", " ", "X", "O", " ", "X", "O", " ", " "]

      expect(won?(board)).to match_array([0,3,6])
    end

  puts  it 'returns an array of matching indexes for a middle column win' do
      board = ["X", "O", " ", "X", "O", " ", " ", "O", " "]

      expect(won?(board)).to match_array([1,4,7])
    end

    puts it 'returns an array of matching indexes for a right column win' do
      board = ["X", " ", "O", "X", " ", "O", " ", " ", "O"]

      expect(won?(board)).to match_array([2,5,8])
    end

    puts it 'returns an array of matching indexes for a left diagonal win' do
      board = ["X", " ", "O", " ", "X", "O", " ", " ", "X"]

      expect(won?(board)).to match_array([0,4,8])
    end

    puts it 'returns an array of matching indexes for a right diagonal win' do
      board = ["X", " ", "O", "X", "O", " ", "O", " ", " "]
  ...
  puts describe '#draw?' do
    ...
    puts it 'returns false for a game won in the first row' do
      board = ["X", "X", "X", "O", "X", "O", "O", "O", "X"]

      expect(draw?(board)).to be_falsey
    end

    puts it 'returns false for a won game diagonaly' do
  ...
  puts describe '#over?' do
    ...
    puts it 'returns true for a won game when the board is full' do
      board = ["X", "O", "X", "O", "X", "X", "O", "O", "X"]

      expect(over?(board)).to be_truthy
    end

    puts it 'returns true for a won game when the board is not full' do
      board = ["X", " ", " ", "O", "O", "O", "X", "X", " "]
  ...
  puts describe '#winner' do
    ...
    puts it 'returns O when O won' do
      board = ["X", "O", " ", " ", "O", " ", " ", "O", "X"]

      expect(winner(board)).to eq("O")
    end

    puts it 'returns nil when no winner' do
      board = ["X", "O", " ", " ", " ", " ", " ", "O", "X"]
